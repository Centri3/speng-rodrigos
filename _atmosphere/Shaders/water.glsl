...
void main()
{
    ...
    for (int i=0; i<NLights; i++)
    {
        ...
        // Ringshine
            #ifdef RINGS
            // Determine the orientation of the ring planex
            float ringNormalL = (SurfParams3.x == 0.0) ? lightVec.y : lightVec.z;
            float ringNormalN = (SurfParams3.x == 0.0) ? Normal.y : Normal.z;
            ...
            float OuterRing = Radiuses.y / (RingsParams.x + (1.0 / RingsParams.w)); // Outer ring rel to planet
            float MeanRing = Radiuses.y / (Radiuses.y + (RingsParams.x + (1.0 / RingsParams.w)) * 0.5); // mean ring rel to planet
            ...
            // Normalize to get pure longitudinal direction 
            // (+ 1e-6 is a tiny epsilon to prevent division by zero exactly at the poles)
            ...
            float nightShadow = min(1, mix(MinIllum, 1, midnightFade));
            // ----------------------------------------------
            ...
            // Add ringshine to the ambient sea term
            ambSeaTerm += ringShine;
            
            // Spec shine, broken not working
            float ringFresnel = 0.02 + 0.98 * pow(1.0 - abs(eyeVecTS.z), 4.0);
            //specSeaAccum += ringShine * ringFresnel*0.02;
            
            // Keep volume scattering (even though it will be dark, it's physically accurate)
            waterFogAccum += ringShine;
            
            // Atmospheric Haze
            #ifdef ATMO
                // Save the Sun's altitude
                float savedEyeMuS = EyeMuS;
                
                // Ambient air on terrain
                EyeMuS = 1.0 - latSine/horizonEnd; 
                
                // Add the isotropic ambient ringshine to the inscattered terrain haze
                if (atmoHorFix)
                    Inscatter += inscatterGroundFix(vec3(0.0)) * ringShine;
                else
                    Inscatter += inscatterGround(vec3(0.0)) * ringShine;
                
                // Restore the Sun's altitude
                EyeMuS = savedEyeMuS;
            #endif
            // ----------------------------
        #endif
