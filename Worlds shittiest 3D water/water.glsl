...
//=============================================================================
// --- 3D WAVE DISPLACEMENT FUNCTION (8 OCTAVES) ---
float getWaveHeight(vec2 uv, float time) {
    float waveScale = 200.0;

    vec2 baseUV = uv * SurfParams3.yz * waveScale;
    
    vec2 uv0 = baseUV * 1.00 + vec2(0.17, 0.65) + vec2(0.8, 0.6)  * (time * 0.40);

    vec2 uv1 = baseUV * 1.70 + vec2(0.65, 0.83) + vec2(-0.6, 0.7) * (time * 0.55);

    vec2 uv2 = baseUV * 2.89 + vec2(0.23, 0.11) + vec2(0.4, -0.5) * (time * 0.75);

    vec2 uv3 = baseUV * 4.91 + vec2(0.88, 0.44) + vec2(-0.3, -0.8)* (time * 0.95);

    vec2 uv4 = baseUV * 8.35 + vec2(0.51, 0.27) + vec2(0.9, 0.2)  * (time * 1.20);

    vec2 uv5 = baseUV * 14.2 + vec2(0.12, 0.94) + vec2(-0.7, -0.4)* (time * 1.50);

    vec2 uv6 = baseUV * 24.1 + vec2(0.76, 0.38) + vec2(0.3, 0.9)  * (time * 1.90);

    vec2 uv7 = baseUV * 41.0 + vec2(0.45, 0.62) + vec2(-0.8, 0.5) * (time * 2.40);

    float h0 = textureLod(NoiseMap, uv0, 0.0).r * 2.0 - 1.0;
    float h1 = textureLod(NoiseMap, uv1, 0.0).r * 2.0 - 1.0;

    float h2 = textureLod(NoiseMap, uv2, 0.0).r * 2.0 - 1.0;
    float h3 = textureLod(NoiseMap, uv3, 0.0).r * 2.0 - 1.0;

    float h4 = textureLod(NoiseMap, uv4, 0.0).r * 2.0 - 1.0;
    float h5 = textureLod(NoiseMap, uv5, 0.0).r * 2.0 - 1.0;

    float h6 = textureLod(NoiseMap, uv6, 0.0).r * 2.0 - 1.0;
    float h7 = textureLod(NoiseMap, uv7, 0.0).r * 2.0 - 1.0;

    float height = h0 * 0.490 
                 + h1 * 0.260 
                 + h2 * 0.130 
                 + h3 * 0.060 
                 + h4 * 0.030 
...
void main()
{
    ...
    float time = 1.0 * SurfParams3.w;
    float wHeight = getWaveHeight(vTexCoord.xy, time);
    fWaveHeight = wHeight / 0.0000006;

    #ifdef VSDBL
        dvec3 dvPosition, dvTangent;
        SphereSegmentVertexCoordD(dvPosition, dvTangent);
        dvec3 dvNormal = normalize(dvPosition);

        dvPosition += dvNormal * double(wHeight);
        ...
        vec3 fvNormal = normalize(fPosition.xyz);

        fPosition.xyz += fvNormal * wHeight;
