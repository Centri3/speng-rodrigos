...
void main() {
  ...
  if (cloudsCoverage == 1.0) {
    ...
    OutColor =
        0.1 +
        _GetGasGiantCloudsColor(max(height * _stripeFluct * 0.5,
                                   1.0 - float(BIOME_CLOUD_LAYERS - 1) /
                                             float(BIOME_SURF_LAYERS))) +
        0.1 * _GetGasGiantCloudsColor(min(height * _stripeFluct * 0.5,
    ...
    OutColor.rgb = mix(_GetCloudsColor(0.0).rgb, OutColor.rgb,
