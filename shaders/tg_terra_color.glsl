...
Surface _GetBaseSurface(float height, vec2 detUV) {
  float h = (height * 2.0 - seaLevel * 2.0) / (1.0 - seaLevel) *
...
void ColorMapTerra(vec3 point, in BiomeData biomeData, out vec4 ColorMap) {
  ...
  if (tidalLock <= 0.0) {
    ...
    if (latitude < latTropic - tropicWidth)
      climate = mix(climateTropic, climateEquator,
                    saturate((latTropic - latitude - tropicWidth) / latTropic));
    else if (latitude > latTropic + tropicWidth)
      climate = mix(
          climateTropic, climatePole,
          saturate((latitude - latTropic - tropicWidth) / (1.0 - latTropic)));
    else
  ...
  if (plantsBiomeOffset > 0.0) {
    ...
    // if (cracksOctaves == 0 && volcanoActivity >= 1.0)
    //	{
    //		humidityMod = (saturate(iqTurbulence(point, 0.55) * (2 *
    //(volcanoActivity - 1))) + humidity * 2.0) * (volcanoActivity - 1) +
    //(Fbm((point + distort) * 1.73) - 1.0 + humidity * 2.0) * (2 -
    // volcanoActivity);  //Io like plants
    //	}
    // else if (cracksOctaves == 0 && volcanoActivity < 1.0)
    //{
    //		humidityMod = Fbm((point + distort) * 1.73) - 1.0 + humidity
    //* 2.0;
    ////Io like plants
    //}
  ...
  if (climate > 0.8 && snowLevel != 2.0) {
    float snowTransition =
        ...
        smoothstep(latTropic - 0.2, latTropic + 0.1,
                   latitude); // * smoothstep(0.75, 0.65, biomeData.slope);
...
#define CLIFF_TRANSITION_END 0.65 // 0.55
  if ((iceCap == 0.0) && (biomeData.slope > CLIFF_TRANSITION_BEGIN)) {
    ...
    noiseOctaves = 4.0;
    float layers = Fbm(vec3(biomeData.height * 168.4 + 0.17 * vary,
                            0.43 * (p.x + p.y), 0.43 * (p.z - p.y)));

    // Get a steep cliff surface and modulate its color by the layer pattern
    Surface cliff = DetailTextureMulti(detUV, BIOME_ROCK);
    // cliff.color.rgb *= vec3(1.0) + 0.5*colorVary * layers;
    cliff.color.rgb *= vec3(1.0) - vec3(0.0, 0.25, 0.5) * layers; // washed red
    // cliff.color.rgb *= vec3(1.0) - vec3(0.45, 0.6, 0.75) * layers; //grey
    // cliff.color.rgb *= vec3(1.0) - vec3(0.0, 0.5, 1.0) * layers; //bright red
    // original

    // Interpolate previous surface to the cliff surface
    float cliffTransition = smoothstep(CLIFF_TRANSITION_BEGIN,
                                       CLIFF_TRANSITION_END, biomeData.slope);
...
//#define VISUALIZE_BIOMES
  ...
  const vec4 BiomeDebugColor[BIOME_SURF_LAYERS] =
      vec4[BIOME_SURF_LAYERS](vec4(0.5, 0.1, 0.5, 0.0), // BIOME_BOTTOM
