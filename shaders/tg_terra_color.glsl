...
//RODRIGO
    ...
    // Ice caps
    float iceCap = saturate((latitude / latIceCaps - 0.982) * 50.0);
...
//RODRIGO - small changes 
    noiseOctaves = 14.0;
    noiseLacunarity = 2.218281828459;
    vec3 pp = (point + Randomize) * (0.0005 * hillsFreq / (max(hillsMagn, .01) * max(hillsMagn, .01)));
...
//RODRIGO -Change albedovary to vary
	...
	{
        surf = obsidian;
		vary = (0 - 20* pow(abs((-JordanTurbulence(point * 10000.0 + Randomize, 1.1, 0.9, 0.9, 0.8, 0.3, 0.3, 1.7) * 1.5) - JordanTurbulence(point * 3.0 + Randomize, 1.1, 0.9, 0.9, 0.8, 0.3, 0.3, -1.7) * 0.3) * 0.28, 1.5));
		//vary = -(globTemp + varyTemp * 0.0)+0.5;;		//-(globTemp + varyTemp * 0.0)+0.5;
	}
