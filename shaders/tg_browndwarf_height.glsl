...
vec3 CycloneNoiseGasGiantGmail(vec3 point, inout float offset) {
    ...
    for(int i = 0; i < cycloneOctaves; i++) {
        cell = inverseSF(vec3(point.x, point.y * squeeze, point.z), freq + cloudsLayer, cellCenter); // cell = Cell3NoiseVec(p * freq, 0.6);   inverseSF(vec3(point.x, point.y * squeeze, point.z), freq + cloudsLayer, cellCenter);
        rnd = hash1(cell.x);
        r = size * cell.y;
