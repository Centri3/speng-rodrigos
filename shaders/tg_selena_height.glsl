...
float   _MareHeightFunc(vec3 point, float lastLand, float lastlastLand, float height, float r, inout float mareFloor)
{
    ...
    {   // crater bottom
		...
		noiseOctaves     = 6.0;
		//noiseLacunarity  = 2.218281828459;      // Causes terrain height offset (cliffs)
		//noiseH           = 0.9;                 // Causes terrain height offset (cliffs)
		noiseOffset      = 0.5;
...
float   HeightMapSelena(vec3 point)
{
    ...
    noiseOctaves = 10.0;
    noiseH = 1.0;
    //noiseLacunarity = 2.3;      // Causes terrain height offset (cliffs)
