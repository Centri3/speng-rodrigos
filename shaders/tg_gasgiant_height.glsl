...
float   HeightMapCloudsGasGiantGmail(vec3 point)
{
    ...
    twistedPoint = twistedPoint * (0.43 * cloudsFreq) + Randomize;
    twistedPoint.y *= 9.0 + turbulence;
    float height = stripeFluct * (Fbm(twistedPoint) * 0.8 + 0.1);
...
float   HeightMapCloudsGasGiantGmail2(vec3 point)
{
    ...
    // Compute zones
    float zones = Noise(vec3(0.0, twistedPoint.y * stripeZones * 0.5, 0.3)) * 0.5 + 0.10;
    float offset = 0.1;
    ...
    twistedPoint = twistedPoint * (0.32 * cloudsFreq) + Randomize;
    twistedPoint.y *= 30.0 + turbulence;
    float height = stripeFluct * (Fbm(twistedPoint) * 0.5 + 0.4);
...
float   HeightMapCloudsGasGiantGmail3(vec3 point)
{
    ...
    // Compute zones
    float zones = Noise(vec3(0.0, twistedPoint.y * stripeZones * 0.9, 0.3)) * 0.4 + 0.08865;
    float offset = 0.0;
    ...
    twistedPoint = twistedPoint * (1.12 * cloudsFreq) + Randomize;
    twistedPoint.y *= 80.0 + turbulence;
    float height = stripeFluct * (Fbm(twistedPoint) * 0.25 + 0.4);
...
void main()
{
    if (cloudsLayer == 0.0)
    {
        ...
        float height = (HeightMapCloudsGasGiantGmail(point) + HeightMapCloudsGasGiantGmail2(point) + HeightMapCloudsGasGiantGmail3(point));
