...
float HeightMapCloudsGasGiantAli(vec3 point) {
  ...
  float turbulence = RidgedMultifractalDetail(twistedPoint * 0.03 + Randomize, stripeZones, stripeFluct);
  ...
  return (zones * stripeFluct) + height + hash1(HASHED_RANDOMIZE) + offset;
}
...
float HeightMapCloudsGasGiantGmail2(vec3 point) {
  ...
  // Compute stripes
  noiseOctaves = cloudsOctaves;
  float turbulence = RidgedMultifractalDetail(twistedPoint * 0.01 + Randomize, stripeZones, stripeFluct);
  ...
  return (zones * stripeFluct) + height + offset;
}
...
float HeightMapCloudsGasGiantGmail3(vec3 point) {
  ...
  // Compute stripes
  noiseOctaves = cloudsOctaves;
  float turbulence = RidgedMultifractalDetail(twistedPoint * 8.86 + Randomize, stripeZones, stripeFluct);
...
float   HeightMapCloudsGasGiantTPE(vec3 point, bool cyclones, float _stripeZones)
{
	vec3  twistedPoint = point;
...
float coverage = cloudsCoverage;
	...
	// Compute zones
	float zones = Noise(vec3(0.0, twistedPoint.y * _stripeZones * 0.5, 0.0)) * 0.6 + 0.25;
	float offset = 0.0;
	...
	// Compute stripes
	noiseOctaves = cloudsOctaves;
	float turbulence = RidgedMultifractalDetail(twistedPoint * 2.2 + Randomize, stripeZones, stripeFluct);
...
void main()
{
	if (cloudsLayer == 0.0)
	{
		...
		{
			height = 3.0 * stripeFluct * HeightMapCloudsVenusAli(point) + HeightMapCloudsVenusAli2(point);
	...
	else
	{
		...
		{
			height = 0.95 * HeightMapCloudsGasGiantTPE(point, true, stripeZones);
		}
		OutColor = vec4(height);
	}
}
