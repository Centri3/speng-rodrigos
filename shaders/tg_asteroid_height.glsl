...
float   HeightMapAsteroid(vec3 point)
{
    ...
    heightPeak  =  0.6;
    heightRim   =  0.4;
    float crater = 0.4 * CraterNoise(point, craterMagn * 0.3, craterFreq * 10.0, craterSqrtDensity, craterOctaves);
    ...
    if (eqridgeMagn > 0.0)
    {
	  ...
	  //eqridgeModFreq 4
	    ...
	    height = max(height + (eqridgeMagn * ridgeHeight * 1.5 * iqTurbulence(point * 0.4 * eqridgeModFreq + Randomize, 0.2 * eqridgeModMagn)), height);
    }
	...
	float vary = (1.0 - 5*(Fbm((point + distort) * (1.5 - RidgedMultifractal(pp, 8.0)+ RidgedMultifractal(pp*0.999, 8.0))))) * 0.001;
