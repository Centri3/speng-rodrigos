...
void _PseudoRivers(vec3 point, float global, float damping,
                   inout float height) {
  noiseOctaves = 8.0;
  noiseH = 1.0;
  noiseLacunarity = 2.1;
  ...
  float rivers =
      1.0 - (saturate(6.5 * abs(cell.y - cell.x) * riversMagn * 0.2));
...
void HeightMapTerra(vec3 point, out vec4 HeightBiomeMap) {
  ...
  noiseOctaves =
      (oceanType == 1.0)
          ? 5.0
          : 12.0; // Reduce terrain octaves on oceanic planets (oceanType == 1)
  noiseH = 0.5;
  ...
  if (riversMagn > 0.0) {
    damping = (smoothstep(0.195, 0.135,
